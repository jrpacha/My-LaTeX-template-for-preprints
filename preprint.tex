...
% Nota: about how to include source code in our LaTeX documents 
...
%\href{mailto:first.author@e.mail}{\texttt{first.author@e.mail}}
\end{minipage}
\begin{minipage}[c]{0.4\linewidth}
\centering
  Second Author$^{2}$\\
  \url{second.author@e.mail}
  %\href{mailto:second.author@e.mail}{\texttt{second.author@e.mail}}
\end{minipage}\\ [0,3in]
\begin{minipage}[c]{0.4\linewidth}
\centering
  Third Author$^{3}$\\
  \url{third.author@e.mail}
  %\href{mailto:third.author@e.mail}{\textt{third.author@e.mail}}
\end{minipage}\\
...
\noindent
Listing~\ref{c-code:area} shows a C-program (see~\cite{KernighanR88}) that, given its sides'
length, computes the triangle's area.
 
\lstinputlisting[style=customc,
		 caption={Computation of a triangle's area given its three sides.},
		 label=c-code:area]{src/area.c}
